 ...
 * @author Mega
 */
abstract class BaseDraw extends \yii\db\ActiveRecord
{
...
//    }
    ...
    protected static function _getWinsStat(BaseLottery $lottery)
    {
        ...
        $stat         = $class::find()
                ...
                ->groupBy('correct_numbers, is_bet_extended, bet_cnt')
                ->having(['>=', 'correct_numbers', 2])
                ->orderBy('correct_numbers, is_bet_extended, bet_cnt')
//                ->with('bet');
                ->asArray()
                ->all();
        $statExtended = [];
        foreach ($stat as $item) {
            if (!isset($statExtended[$item['correct_numbers']]['winners'])) {
                $statExtended[$item['correct_numbers']]['winners'] = 0;
            }
            if ($class::MIN_NUMBERS == $item['correct_numbers']) {
                $statExtended[$class::MIN_NUMBERS]['winners'] += $item['winners'];
            } elseif ((bool) $item['is_bet_extended']) {
                for ($correctNumbers = 2; $correctNumbers <= $item['correct_numbers']; $correctNumbers++) {
                    $level = $item['correct_numbers'] - $correctNumbers + 1;
                    if (!isset($statExtended[$correctNumbers]['winners'])) {
                        $statExtended[$correctNumbers]['winners'] = 0;
                    }
                    $statExtended[$correctNumbers]['winners'] +=$class::betWinsStat($item['bet_cnt'], $item['correct_numbers'], $level) * $item['winners'];
                }
            } else {
                $statExtended[$item['correct_numbers']]['winners'] +=$item['winners'];
            }
        }
        ksort($stat);
        ksort($statExtended);
     ...
     */
    protected static function _paidOutTickets($paidOutOne, $correctNumbersCount, $lotteryId, $bet_cnt)
    {
        $class = __NAMESPACE__ . '\\L' . static::ID . 'Ticket';
        //http://stackoverflow.com/questions/25469689/yii2-update-field-with-query-builder
        return Yii::$app->db->createCommand()
                        ->update(
                                $class::tableName(), ['paid_out' => $paidOutOne], ['win_cnt' => $correctNumbersCount, 'lottery_id' => $lotteryId, 'bet_cnt' => $bet_cnt]
                        )
                        ->execute();
    }
...
//    REPLACE INTO lottery_user_referrals_stat(lottery_id, user_id, level, paid_in, paid_out) SELECT d.lottery_id, a.id, 1, SUM(paid), SUM(paid_out) FROM lottery_user as a INNER JOIN lottery_user as b on a.referral_id = b.referrer_id inner join lottery_l6x45_ticket as d on d.user_id = b.id group by a.id
    ...
    public static function _paidOutRefs(BaseLottery $lottery)
    {
        $class      = __NAMESPACE__ . '\\L' . static::ID . 'Ticket';
        $refRate    = [];
        $refRate[0] = Yii::$app->keyStorage->get('user.account.ref_rate_leader') / 100; //Начисления лидеру
...
//        insert into lottery_user_referrals_stat(lottery_id, user_id, lottery_type, level, ref_count, paid_in, paid_out) SELECT d.lottery_id, a.id, '6x45', 3, count(DISTINCT e.id), SUM(paid), SUM(paid_out) FROM lottery_user as a INNER JOIN lottery_user as b on a.referral_id = b.referrer_id inner join lottery_user as c on b.referral_id = c.referrer_id inner join lottery_user as e on c.referral_id = e.referrer_id inner join lottery_l6x45_ticket as d on d.user_id = e.id where lottery_id = 7 group by a.id
        ...
        //Реф статистика лидера
        $sql                = "INSERT INTO {{%user_referrals_stat}}(lottery_id, user_id, lottery_type, level, ref_count, paid_in, paid_out, paid_ref) SELECT d.lottery_id, a.id, :type, 0, count(DISTINCT b.id), SUM(paid), SUM(paid_out), SUM(paid) * :refRate FROM {{%user}} as a INNER JOIN {{%user}} as b on a.referral_id = b.leader_id inner join $table as d on d.user_id = b.id WHERE d.lottery_id = :id group by a.id";
        $params[':refRate'] = $refRate[0];
        $res                = $res + Yii::$app->db->createCommand($sql)->bindValues($params)->execute();
...
//        $res    = $res + Yii::$app->db->createCommand($sql)->bindValues($params)->execute();
        ...
        foreach ($payments as $payment) {
            ...
            if ($userProfile) {
                $userProfile->AccountCharge($payment['paid'], 0 == $payment['level']?'Реферальные начисления главе сообщества':'Реферальные начисления');
                $userProfile->save();
            }
        }
